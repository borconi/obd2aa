...
public class isCameraInWay {
    ...
    public isCameraInWay(Location location, CameraDataBaseHelper mobileDB, CameraDataBaseHelper staticDB, int seacrhdistance, String mobilefilter, String staticfilter) {
        ...
        if (OBD2Background.isdebugging) {
            ...
            Log.d("OBD2AA", "Tirangle 1:" + SphericalUtil.computeOffset(car_possiotion, seacrhdistance, location.getBearing() - 15));
            Log.d("OBD2AA", "Tirangle 2:" + SphericalUtil.computeOffset(car_possiotion, seacrhdistance, location.getBearing() + 15));
        }
        latLngs.add(SphericalUtil.computeOffset(car_possiotion, seacrhdistance, location.getBearing() - 15));
        latLngs.add(SphericalUtil.computeOffset(car_possiotion, seacrhdistance, location.getBearing() + 15));
    ...
    public static boolean stillinRange(Location location, NearbyCameras camera, int seacrhdistance) {
        ...
        latLngs.add(SphericalUtil.computeOffset(current_possition, seacrhdistance, location.getBearing() - 15));
        latLngs.add(SphericalUtil.computeOffset(current_possition, seacrhdistance, location.getBearing() + 15));
